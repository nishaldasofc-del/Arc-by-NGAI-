...
// NOTE: callAI, detectIntent, generateQuestions, generateFullPlan,
// generateImplementationSteps, and extractChunkText have been removed.
// The Vercel AI SDK (useChat) and the /api/chat backend route now handle
// all AI streaming. See api/chat.ts.




// ─── Build System Prompt ──────────────────────────────────────────────────────
// NOTE: This prompt is kept here for reference. Move it to api/chat.ts as the
// `system` string when you set up the backend route.
...
function buildPreviewHtml(files: ProjectFile[]): string {
  ...
  // Inject CSS — replace <link> tag if found, else inject before </head>, else prepend
  files.filter(f => f.language === 'css').forEach(cssFile => {
    const linkPattern = new RegExp(`<link[^>]*href=["']${escapeRegex(cssFile.name)}["'][^>]*>`, 'gi');
    const inlineStyle = `<style>\n${cssFile.content}\n</style>`;
    if (linkPattern.test(html)) {
      html = html.replace(
        new RegExp(`<link[^>]*href=["']${escapeRegex(cssFile.name)}["'][^>]*>`, 'gi'),
        inlineStyle
      );
  ...
  // Inject JS — replace <script src=""> if found, else inject before </body>, else append
  files.filter(f => f.language === 'js').forEach(jsFile => {
    ...
    if (scriptPattern.test(html)) {
      html = html.replace(
        new RegExp(`<script[^>]*src=["']${escapeRegex(jsFile.name)}["'][^>]*>\\s*<\\/script>`, 'gi'),
        inlineScript
      );
  ...
  return (
    <div className="w-full max-w-[80%] flex flex-col gap-1.5">
      ...
      <div className="text-sm text-gray-800 dark:text-gray-200 whitespace-pre-wrap leading-relaxed">
        {display || ''}
        <span className="inline-block w-[3px] h-[1em] bg-violet-500/70 ml-0.5 animate-pulse align-middle rounded-sm" />
      </div>
    </div>
  );
}
...
export default function Builder() {
  ...
  const {
    ...
    api: '/api/chat',
    onResponse: (res) => {
      if (!res.ok) console.error('Error from API:', res.status);
    },
    onFinish: (message) => {
      // Parse any files from the finished AI message and update project state
      const files = parseFilesFromOutput(message.content);
      if (files.length > 0) {
        ...
        pushHistory(files, input.slice(0, 40));
        addLog('system', `Done: ${files.length} files in ${elapsed}s.`);
      }
  ...
  // Bridge: keep a local messages state that merges SDK messages with our
  // custom Message type for UI rendering (questions, plan cards, etc.)
...
Output ONLY these missing files using the === FILE: name === ... === END FILE === format. Do not repeat already generated files.`;
  ...
  // ── Submit ─────────────────────────────────────────────────────────────────
  // handleSubmit now uses the Vercel AI SDK's sdkHandleSubmit for all AI calls.
  // The old detectIntent / generateQuestions / runBuild / runChat branches have
  // been consolidated: we pass the user message straight to /api/chat and let
  // the backend decide intent. Custom UI state (building indicators, file parsing)
  // is handled in the useChat onFinish callback above.
  const handleSubmit = (e?: React.FormEvent) => {
    e?.preventDefault();
    const text = input.trim();
    if (!text || isStreaming) return;

    // Add the user message to our local display list
    setMessages(prev => [...prev, {
    ...
    // Fire the SDK request — streams to /api/chat
    sdkHandleSubmit(e as React.FormEvent<HTMLFormElement>);

    // Remove thinking dot once SDK starts streaming (small delay)
    setTimeout(() => {
  ...
  return (
    <>
      <style>{`
        ...
        *, *::before, *::after { font-family: 'Geist', ui-sans-serif, system-ui, -apple-system, sans-serif !important; }
        code, pre, textarea, .font-mono { font-family: 'Geist Mono', ui-monospace, 'Cascadia Code', monospace !important; }
          ...
          <div className="flex flex-1 min-h-0 relative">
            ...
            <div className={cn(
                      ...
                      <div key={msg.id} className={cn('flex', msg.role === 'user' ? 'justify-end' : 'justify-start')}>
                        ...
                        {msg.role === 'agent' && msg.status === 'done' && !msg.planCard && !msg.questions && !msg.fullPlan && !msg.implementationSteps && msg.content && (
                          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                            className="w-full max-w-[80%] flex flex-col gap-1.5">
                            {(() => {
              ...
              <div className={cn(
                ...
                <form onSubmit={handleSubmit} className={cn('flex flex-col gap-2 rounded-2xl arc-input-glass p-3')}>
                  <textarea
                    ref={textareaRef}
                    value={input}
