...
type AgentPhase =
  | 'idle' | 'questions' | 'planning' | 'plan_review' | 'building' | 'done';
...
async function callAI(prompt: string, maxTokens = 1500, systemPrompt?: string): Promise<string> {
  ...
  const res = await fetch('https://zenoai-1.onrender.com/api/v1/chat/stream', {
    ...
    body: JSON.stringify({ session_id: `util-${Date.now()}`, message: body.messages[0].content }),
  });
...
async function generateQuestions(userText: string): Promise<AIQuestion[]> {
...
async function generateFullPlan(userText: string, answers: Record<string, string>): Promise<FullPlan | null> {
...
Generate the complete project with SEPARATE HTML, CSS, and JS files now:`;
}
...
export default function Builder() {
  ...
  const handleQuestionsSubmit = async (answers: Record<string, string>) => {
    ...
    setMessages(prev => [...prev, {
      id: planningMsgId,
      role: 'agent',
      content: 'Let me create a proper implementation plan for this…',
      status: 'planning',
      timestamp: new Date(),
    }]);
    ...
    const plan = await generateFullPlan(pendingBuildText, answers);
    ...
    if (plan) {
      ...
      setMessages(prev => prev.map(m =>
        m.id === planningMsgId
          ? { ...m, status: 'plan_review', fullPlan: plan }
          : m
      ));
  ...
  const handlePlanApprove = async () => {
    if (!pendingBuildText) return;
    setMessages(prev => prev.map(m =>
      m.status === 'plan_review' ? { ...m, status: 'done' } : m
    ));
    ...
    try {
      ...
      while (true) {
        ...
        for (const line of lines) {
          ...
          // Real-time file detection - advance steps as files are generated
          ...
          if (partialFiles.length > lastParsedFileCount) {
            ...
            } else if (currentStepIdx < implSteps.length - 1 && partialFiles.length > lastParsedFileCount + 1) {
              currentStepIdx++;
  ...
  const handleSubmit = async () => {
    ...
    if (intent === 'chat') {
      ...
      // NEW FLOW: Questions → Planning → Plan Review → Build
      setPendingBuildText(text);
      setAgentPhase('questions');

      // Show "gathering context" message
      setMessages(prev => [...prev, {
        id: `arc-intro-${Date.now()}`,
        role: 'agent',
        content: "I need a few details before I start building",
        status: 'done',
        timestamp: new Date(),
      }]);
                        ...
                        {msg.role === 'agent' && msg.status === 'planning' && (
                          <div className="space-y-2 w-full max-w-sm">
                            <motion.div initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }}
                              ...
                              <p className="text-xs text-gray-700 dark:text-gray-300 arc-glass px-3 py-2 rounded-xl rounded-tl-sm">
                                {msg.content}
                              </p>
                            </motion.div>
                            <PlanningStep isActive={true} isDone={false} />
                          </div>
                        )}
              ...
              <div className={cn(
                ...
                <div className={cn('flex flex-col gap-2 rounded-2xl arc-input-glass p-3', isStreaming && 'opacity-80')}>
                  <textarea
                    ...
                    placeholder={isStreaming ? 'Arc is working…' : agentPhase === 'questions' ? 'Answer the questions above or type something…' : projectFiles.length > 0 ? 'Ask for changes, edits, or a new project…' : 'Describe what you want to build…'}
                    rows={1}
