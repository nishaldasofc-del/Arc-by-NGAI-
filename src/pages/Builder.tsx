...
async function generatePlan(userText: string): Promise<{ summary: string; steps: string[]; clarifyQ?: string } | null> {
  const prompt = `You are an AI coding agent. The user wants to build: "${userText}"

Generate a concise build plan as JSON. Return ONLY valid JSON, no markdown:
{
  "summary": "One sentence describing what you'll build",
  "steps": ["step1", "step2", "step3", "step4"],
  "clarifyQ": "One optional clarifying question if genuinely needed, or omit this field"
}

Steps should be 3-5 items, each under 8 words. Only include clarifyQ if you truly need more info to build correctly.`;
...
function buildPreviewHtml(files: ProjectFile[]): string {
  ...
  if (!htmlFile) {
    // Try to find any file that looks like HTML by content
...
function PlanCard({ plan, onConfirm, onEdit }: {
  plan: { summary: string; steps: string[]; clarifyQ?: string };
  onConfirm: () => void;
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
      ...
      <div className="px-4 py-3 border-b border-gray-200 dark:border-white/[0.06] flex items-center gap-2">
        ...
        <span className="text-xs font-semibold text-gray-700 dark:text-gray-300">Build Plan</span>
      </div>
      <div className="px-4 py-3 space-y-3">
        <p className="text-sm text-gray-700 dark:text-gray-300 leading-relaxed">{plan.summary}</p>
        ...
        {plan.clarifyQ && (
          <div className="bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20 rounded-xl px-3 py-2.5 space-y-2">
            <p className="text-xs text-amber-700 dark:text-amber-300 font-medium">{plan.clarifyQ}</p>
            <div className="flex gap-2">
              <input
                value={clarify}
                onChange={e => setClarify(e.target.value)}
                onKeyDown={e => { if (e.key === 'Enter' && clarify.trim()) onEdit(clarify.trim()); }}
                placeholder="Type your answer…"
                className="flex-1 bg-white dark:bg-black/30 border border-amber-200 dark:border-amber-500/30 rounded-lg px-2.5 py-1.5 text-xs text-gray-900 dark:text-white placeholder:text-gray-400 outline-none"
              />
              <button onClick={() => clarify.trim() && onEdit(clarify.trim())}
                className="px-2.5 py-1.5 rounded-lg bg-amber-500 text-white text-xs font-medium hover:bg-amber-600 transition-colors">
        ...
        <button onClick={onConfirm}
          className="w-full py-2 rounded-xl arc-btn-3d-primary text-white text-xs font-semibold flex items-center justify-center gap-1.5">
          <Rocket className="w-3.5 h-3.5" /> Build it
        </button>
      </div>
    </motion.div>
  );
...
function StreamingMessage({ content }: { content: string }) {
  ...
  // Check if we're mid-think (tag opened but not closed)
...
export default function Builder() {
  ...
  const runBuild = async (userText: string, intent: 'build' | 'edit') => {
    ...
    try {
      ...
      if (preview) {
        ...
        // Fallback: use raw streamed text as HTML
  ...
  const handleSubmit = async () => {
    ...
    if (intent === 'chat') {
      ...
      // First build: show plan card
      const planMsgId = `plan-${Date.now()}`;
      const plan = await generatePlan(text);
      if (plan) {
        setPendingBuildText(text);
        setMessages(prev => [...prev, {
          id: planMsgId, role: 'agent', content: '', status: 'done',
          timestamp: new Date(), planCard: plan,
        }]);
      } else {
        await runBuild(text, 'build');
      }
    } else {
        ...
        <div className="flex flex-col flex-1 min-w-0 h-full">
          ...
          <div className="flex flex-1 min-h-0 relative">
            ...
            <div className={cn(
              ...
              isMobile ? 'w-full' : messages.length > 0 ? 'w-[380px] shrink-0' : 'flex-1'
            )}>
              {/* Messages — hidden on mobile when viewing preview */}
              <div className={cn(
                  ...
                  <>
                    {messages.map((msg) => (
                      <div key={msg.id} className={cn('flex', msg.role === 'user' ? 'justify-end' : 'justify-start')}>
                        ...
                        {/* Plan card */}
                        {msg.role === 'agent' && msg.status === 'done' && msg.planCard && (
                          <PlanCard
                            plan={msg.planCard}
                            onConfirm={() => {
                              setMessages(prev => prev.filter(m => m.id !== msg.id));
                              if (pendingBuildText) runBuild(pendingBuildText, 'build');
                            }}
                            onEdit={(clarification) => {
                              setMessages(prev => prev.filter(m => m.id !== msg.id));
                              const combined = pendingBuildText
                                ? `${pendingBuildText}. Additional details: ${clarification}`
                                : clarification;
                              setPendingBuildText(null);
                              runBuild(combined, 'build');
                            }}
                          />
                        )}
